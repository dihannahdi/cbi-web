...
export async function generateMetadata({
  ...
  const strapiData = await fetchStrapiProduct("biokalsi-dolomit-pembenah-tanah", lang);
  const data = strapiData || productData[lang]; // Fallback to static if Strapi unavailable
  
  // Extract keywords from focus_keyphrase if available
  const keywords = strapiData?.focusKeyphrase 
    ? strapiData.focusKeyphrase.split(',').map(k => k.trim())
    : (lang === 'id'
  ...
  return {
    ...
    openGraph: {
      ...
      images: strapiData?.heroImage?.url 
        ...
        : ['/images/products/biokalsi-dolomit.webp'],
      type: 'website',
    },
    alternates: {
      canonical: `${SITE_CONFIG.url}/${lang}/produk-layanan/pertanian/biokalsi-dolomit-pembenah-tanah`,
      languages: {
        'id': `${SITE_CONFIG.url}/id/produk-layanan/pertanian/biokalsi-dolomit-pembenah-tanah`,
        'en': `${SITE_CONFIG.url}/en/produk-layanan/pertanian/biokalsi-dolomit-pembenah-tanah`,
      },
    },
  };
}
...
export default async function BiokalsiPage({
  ...
  const strapiData = await fetchStrapiProduct("biokalsi-dolomit-pembenah-tanah");
  ...
  const imageSchema = generateImageObjectSchema({
    url: '/images/products/biokalsi-dolomit.webp',
    name: `BIOKALSI - ${lang === 'id' ? 'Dolomit Pembenah Tanah' : 'Dolomite Soil Conditioner'}`,
    caption: lang === 'id' 
      ? 'BIOKALSI Dolomit - Pembenah Tanah untuk Netralkan pH'
      : 'BIOKALSI Dolomite - Soil Conditioner to Neutralize pH',
    description: lang === 'id'
      ? 'Gambar produk BIOKALSI dolomit pembenah tanah dengan kandungan CaO dan MgO tinggi'
      : 'BIOKALSI dolomite soil conditioner product image with high CaO and MgO content',
    width: 600,
    height: 600,
    encodingFormat: 'image/webp',
    representativeOfPage: true,
    keywords: ['biokalsi', 'dolomit', 'dolomite', 'pembenah tanah', 'centra biotech'],
  });
