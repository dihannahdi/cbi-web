...
// Icon mapping helper for livestock
...
function getProductHeroImage(slug: string): string {
  const heroImages: Record<string, string> = {
    "biomaxi": "/images/livestock/biomaxi-hero.jpg",
  };
...
function getProductMainImage(slug: string): string {
  const productImages: Record<string, string> = {
    "biomaxi": "/images/livestock/biomaxi-product.png",
  };
...
async function getProductBySlug(slug: string, locale: string = 'en'): Promise<ProductData | null> {
  try {
    ...
    const res = await fetch(
      `${STRAPI_URL}/api/product-detail-pages?filters[slug][$eq]=${slug}&filters[category][$eq]=livestock&${populateQuery}&locale=${locale}`,
      { 
        next: { revalidate: 60 },
        headers: {
    ...
    if (!res.ok) {
      console.error(`Failed to fetch livestock product: ${res.status}`);
      return null;
    }
    ...
    if ((!data.data || data.data.length === 0) && locale === 'id') {
      console.warn(`[Strapi] Livestock product ${slug} not found in ${locale}, trying English fallback`);
      const fallbackRes = await fetch(
        `${STRAPI_URL}/api/product-detail-pages?filters[slug][$eq]=${slug}&filters[category][$eq]=livestock&${populateQuery}&locale=en`,
        { 
          next: { revalidate: 60 },
    ...
    console.error("Error fetching livestock product:", error);
    return null;
  }
}

// Get all livestock product slugs for static generation
async function getAllProductSlugs(): Promise<string[]> {
  try {
    const res = await fetch(
      `${STRAPI_URL}/api/product-detail-pages?filters[category][$eq]=livestock&fields[0]=slug`,
      { next: { revalidate: 60 } }
    );
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: pageUrl,
      languages: {
        id: `${baseUrl}/id/produk-layanan/peternakan/${slug}`,
        en: `${baseUrl}/en/produk-layanan/peternakan/${slug}`,
      },
    },
    openGraph: {
...
// Main component
export default async function LivestockProductDetailPage({
  params,
}: {
  params: Promise<{ lang: Locale; slug: string }>;
}) {
  ...
  // Breadcrumb data
  const categoryName = lang === 'id' ? 'Peternakan' : 'Livestock';
  const breadcrumbItems = [
    { name: lang === 'id' ? "Produk & Layanan" : "Products & Services", url: `/${lang}/produk-layanan` },
    { name: categoryName, url: `/${lang}/produk-layanan/peternakan` },
    { name: product.name, url: `/${lang}/produk-layanan/peternakan/${slug}` },
  ];
  ...
  const productSchema = generateProductSchema({
    ...
    url: `${SITE_CONFIG.url}/${lang}/produk-layanan/peternakan/${slug}`,
    offers: {
      price: 0,
      priceCurrency: "IDR",
  ...
  schemas.push(generateImageObjectSchema({
    ...
    keywords: [product.name.toLowerCase(), product.slug, 'centra biotech', 'probiotik ternak'],
  }));
  ...
  return (
    <main className="min-h-screen bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {product.benefits.map((benefit) => {
                ...
                return (
                  ...
                  >
                    <div 
                      className="w-12 h-12 rounded-lg flex items-center justify-center mb-4 bg-blue-100"
                    >
                      <IconComponent 
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {product.certifications.map((cert) => {
                ...
                return (
                  <div
                    key={cert.id}
                    className="bg-white border border-gray-200 p-4 rounded-xl text-center hover:border-blue-500 transition-colors"
                  >
              ...
              <Accordion type="single" collapsible className="space-y-4">
                {product.faq.map((item, index) => (
                  <AccordionItem
                    key={item.id}
                    value={`item-${index}`}
                    className="bg-white border border-gray-200 rounded-xl px-6 data-[state=open]:border-blue-500"
                  >
      ...
      <section 
        className="py-16"
        style={{ backgroundColor: product.primaryColor || '#2563eb' }}
      >
        <ContainerSection>
